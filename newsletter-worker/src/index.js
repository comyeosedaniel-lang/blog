...
function html(body, status = 200) {
  return new Response(
    `<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${SITE_TITLE}</title>
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/subscribe' && request.method === 'POST') {
      ...
      if (existing) {
        await env.DB.prepare('UPDATE subscribers SET confirm_token = ?1 WHERE id = ?2')
          .bind(confirmToken, existing.id)
          .run();
      } else {
        await env.DB.prepare(
          'INSERT INTO subscribers (email, confirm_token, unsubscribe_token) VALUES (?1, ?2, ?3)',
        )
    ...
    if (url.pathname === '/confirm' && request.method === 'GET') {
      ...
      const row = token
        ? await env.DB.prepare('SELECT id FROM subscribers WHERE confirm_token = ?1').bind(token).first()
        : null;

      if (!row) {
        return html('<h1>유효하지 않은 링크예요</h1><p><a href="https://blog.mylineal.com/">홈으로 돌아가기</a></p>', 400);
      }
    ...
    if (url.pathname === '/unsubscribe' && request.method === 'GET') {
      ...
      return html('<h1>구독이 취소됐어요</h1><p>다시 구독하고 싶으시면 언제든 블로그에서 신청해주세요.</p>');
    }
      ...
      for (const sub of results) {
        ...
        await sendEmail(env, {
          ...
          htmlBody: `
            ...
            <h2>${title}</h2>
            ${excerpt ? `<p>${excerpt}</p>` : ''}
            <p><a href="${postUrl}" style="display:inline-block;padding:0.6em 1.2em;background:#af4c26;color:#fff;text-decoration:none;border-radius:6px;">읽으러 가기</a></p>
            <hr style="margin:2em 0;border:none;border-top:1px solid #eee;">
            <p style="font-size:0.85em;color:#888;"><a href="${unsubscribeUrl}">구독 취소</a></p>
          `,
        });
        sent += 1;
      }
